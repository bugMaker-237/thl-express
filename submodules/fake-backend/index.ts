...
createServer((request, response) => {
    ...
    if (route == null) noConfigFound(request, response);
    else {
...
function noConfigFound(request: IncomingMessage, response: ServerResponse) {
    const err = 'Could not find endpoint or route for this request: ' + request.url;
...
function checkRouteParamMatch(request: IncomingMessage, route: Route, cb: (resp: RouteResponse) => void) {
    const data = [];
